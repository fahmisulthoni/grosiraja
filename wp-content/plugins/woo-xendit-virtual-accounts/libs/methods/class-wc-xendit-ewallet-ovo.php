...
class WC_Xendit_OVO extends WC_Xendit_EWallet {
    public function __construct() {
        ...
        $this->method_description   = sprintf( __( 'Collect payment from OVO account on checkout page and get the report realtime on your Xendit Dashboard. <a href="%1$s" target="_blank">Sign In</a> or <a href="%2$s" target="_blank">sign up</a> on Xendit and integrate with <a href="%3$s" target="_blank">your Xendit keys</a>.', 'woocommerce-gateway-xendit' ), 'https://dashboard.xendit.co/auth/login', 'https://dashboard.xendit.co/register', 'https://dashboard.xendit.co/settings/developers#api-keys' );
    ...
    public function payment_fields() {
        ...
        echo '<div class="form-row form-row-wide">
                <input id="'. $this->id .'_phone" type="text" autocomplete="off" placeholder="Phone Number">
            </div>
            <div class="clear"></div>';
    ...
    public function validate_fields(){
        if( empty( $_POST[ $this->id . '_phone' ]) ) {
            wc_add_notice(  '<strong>'. $this->method_code .' phone number</strong> is required!', 'error' );
            return false;
        }
        return true;
    }
