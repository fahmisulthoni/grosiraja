...
class WC_Xendit_CC_Addons extends WC_Xendit_CC {
	...
	public function process_subscription_payment( $order = '', $amount = 0 ) {
		...
		if ( $amount * 100 < WC_Xendit_CC::DEFAULT_MINIMUM_AMOUNT ) {
			return new WP_Error( 'xendit_error', sprintf( __( 'Sorry, the minimum allowed order total is %1$s to use this payment method.', 'woocommerce-gateway-xendit' ), wc_price( WC_Xendit_CC::DEFAULT_MINIMUM_AMOUNT / 100 ) ) );
		}
		...
		// Make the request
		$request             = $this->generate_payment_request( $order, $source->source, null, false, 'true' );

		$this->log('subscription request is -> ' . print_r($request, true));

		$response            = $this->xenditClass->request( $request );

		if ($response->error_code === 'AUTHENTICATION_ID_MISSING_ERROR') {
			return new WP_Error( 'xendit_error', __( 'Your 3DS is activated, please contact us (Xendit customer service) to use subscription with this payment method', 'woocommerce-gateway-xendit' ) );
		}

		if ($response->error_code === 'EXTERNAL_ID_ALREADY_USED_ERROR') {
			$response = $this->xenditClass->request($this->generate_payment_request( $order, $source->source, null, true, 'true' ));
		}

		// Process valid response
		if ( is_wp_error( $response ) ) {
			$this->log('CAUGHT ON IS_WP_ERROR');
		}
	...
	public function process_pre_order( $order_id, $retry, $force_customer ) {
		...
		if ( WC_Pre_Orders_Order::order_requires_payment_tokenization( $order_id ) ) {
			try {
				...
				if ( $order->get_total() * 100 < WC_Xendit_CC::DEFAULT_MINIMUM_AMOUNT ) {
					throw new Exception( sprintf( __( 'Sorry, the minimum allowed order total is %1$s to use this payment method.', 'woocommerce-gateway-xendit' ), wc_price( WC_Xendit_CC::DEFAULT_MINIMUM_AMOUNT / 100 ) ) );
				}
	...
	public function process_pre_order_release_payment( $order ) {
		...
		try {
			...
			while ( 1 ) {
				$source   = $this->get_order_source( $order );
				$response = $this->xenditClass->request( $this->generate_payment_request( $order ) );
